...
export default function AccessGate({ toolKey, toolName, children }: Props) {
  ...
  async function verify(input: string, silent = false, fallback: ClientInfo | null = null) {
    ...
    try {
      ...
      if (data.valid) {
        ...
        setStatus("granted");
        localStorage.setItem(`pds-access-${toolKey}`, JSON.stringify({ code: input, client: data.client }));
      } else {
        if (silent && fallback) {
          setClient(fallback);
          setStatus("granted");
      ...
      if (silent && fallback) { setClient(fallback); setStatus("granted"); }
  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center sm:p-10">
      <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e8f4f5] text-2xl">
        🔐
      </div>
      ...
      <p className="mb-6 text-sm text-slate-500">
        ...
        <span className="font-medium text-[#507c80]">
          No code? Contact Prowess Digital Solutions.
        </span>
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        {status === "denied" && (
          <p className="text-sm font-medium text-red-600">
            {REASON_MSG[reason] ?? REASON_MSG.invalid}
          </p>
        )}
