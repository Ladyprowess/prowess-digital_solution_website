...
export default function ResourceUploadPage() {
  ...
  async function onUpload() {
    ...
    const maxCoverBytes = MAX_COVER_MB * 1024 * 1024;
  
    if (file.size > maxFileBytes) {
      return alert(`Resource file is too large. Max allowed is ${MAX_FILE_MB}MB.`);
    }
  
    if (cover && cover.size > maxCoverBytes) {
      return alert(`Cover image is too large. Max allowed is ${MAX_COVER_MB}MB.`);
    }
